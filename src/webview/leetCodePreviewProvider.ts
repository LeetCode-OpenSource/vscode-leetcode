...
class LeetCodePreviewProvider extends LeetCodeWebview {
  ...
  protected getWebviewContent(): string {
    ...
    const tags: string = [
      ...
      markdownEngine.render(
        this.description.tags.map((t: string) => `[\`${t}\`](${this.getTagLink(t)})`).join(" | ")
      ),
      `</details>`,
    ].join("\n");
    ...
    const links: string = markdownEngine.render(
      `[Submission List](${this.getSubmissionListLink(url)}) | [Solution](${this.getSolutionLink(
        url
      )})`
    );
    return `
  ...
  private getSubmissionListLink(url: string): string {
