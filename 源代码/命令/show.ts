...
async function fetchProblemLanguage(): Promise<string | undefined> {
  ...
  const language: string | undefined =
    defaultLanguage || (await vscode.window.showQuickPick(languages, { placeHolder: "Select the language you want to use", ignoreFocusOut: true }));
...
async function getFinalPath(node: IProblem, leetCodeConfig: vscode.WorkspaceConfiguration, language: string) {
...
async function showProblemWithOneLanguage(node: IProblem, leetCodeConfig: vscode.WorkspaceConfiguration, language: string) {
  let finalPath = await getFinalPath(node, leetCodeConfig, language);
  ...
  if (!fs.existsSync(finalPath)) {
    ...
    return [finalPath, descString];
  } else {
    return ["", ""];
  }
}
function showMarkdownDescription(finalPath, descString) {
  finalPath = path.join(path.dirname(finalPath), path.parse(finalPath).name + ".md");
  if (!fs.existsSync(finalPath)) {
    fs.open(finalPath, "w+", function () {
      fs.writeFile(finalPath, html2md(descString), err => {
        if (err) {
          console.error(err);
        }
...
async function showProblemInternal(node: IProblem): Promise<void> {
  try {
    const leetCodeConfig: vscode.WorkspaceConfiguration = vscode.workspace.getConfiguration("leetcode");
    const languageList: string[] | undefined = await leetCodeConfig.get("languageList");
    const showMD: boolean | undefined = await leetCodeConfig.get("showMD");
    let isShowMD = false;
    if (languageList && languageList.length > 0) {
      languageList.forEach(language => {
        showProblemWithOneLanguage(node, leetCodeConfig, language).then(res => {
          let [finalPath, descString] = res;
          if (showMD && !isShowMD) {
            isShowMD = true;
            showMarkdownDescription(finalPath, descString);
          }
        });
      });
    } else {
      ...
      showProblemWithOneLanguage(node, leetCodeConfig, language).then(res => {
        if (showMD) {
          showMarkdownDescription(res[0], res[1]);
        }
      });
    }
  } catch (error) {
